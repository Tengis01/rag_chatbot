...
- Mobile APK release preparation is validated locally and linked to `@tengis08/document-rag-chatbot` (non-secret EAS project ID in `apps/mobile/app.json`). The mobile-directory `eas.json` has an Android internal-distribution APK profile that bakes in `https://api.ragchatbot.dev` without changing Expo Go's ignored local-IP `.env`. The first signed Android EAS build `6402c25e-968c-40fe-a01b-9f9232b7d371` failed at native compile because Document Picker 56 was incompatible with Expo SDK 53 (ERR-084). The mobile dependency is now `expo-document-picker@13.1.6`; Expo Doctor passes 18/18, mobile typecheck and production Android export pass, and the bundle embeds the production API URL. Commit this minimal repair and submit a replacement signed EAS build, then download, complete installed-device production acceptance, and create the public GitHub Release. See `docs/MOBILE_APK_RELEASE.md`.
