...
- Mobile APK release preparation is validated locally and linked to `@tengis08/document-rag-chatbot` (non-secret EAS project ID in `apps/mobile/app.json`). The mobile-directory `eas.json` has an Android internal-distribution APK profile that bakes in `https://api.ragchatbot.dev` without changing Expo Go's ignored local-IP `.env`. EAS confirms the Android APK profile, package `com.tengis.ragchatbot`, and production API URL. Production-configured Expo export, full workspace build, typecheck, report rebuild and PDF page inspection pass. Signed Android EAS build `6402c25e-968c-40fe-a01b-9f9232b7d371` was submitted from commit `5a12995` and is queued; next is download, installed-device production acceptance, then public GitHub Release. See `docs/MOBILE_APK_RELEASE.md`.
