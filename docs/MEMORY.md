...
- Mobile APK release preparation is validated locally and linked to `@tengis08/document-rag-chatbot` (non-secret EAS project ID in `apps/mobile/app.json`). The mobile-directory `eas.json` has an Android internal-distribution APK profile that bakes in `https://api.ragchatbot.dev` without changing Expo Go's ignored local-IP `.env`. Both initial EAS builds failed because monorepo autolinking generated obsolete `expo.core.ExpoModulesPackage`, even after SDK-compatible Document Picker 13.1.6 fixed Expo Doctor (ERR-084). `apps/mobile/react-native.config.js` now explicitly keeps Expo SDK 53's `expo.modules.ExpoModulesPackage`; isolated resolved config inspection confirms that exact import/instance. Submit one replacement signed build from the committed override; on success download, complete installed-device production acceptance, then create the public GitHub Release. See `docs/MOBILE_APK_RELEASE.md`.
