...
- Mobile APK release preparation is validated locally and linked to `@tengis08/document-rag-chatbot` (non-secret EAS project ID in `apps/mobile/app.json`). The mobile-directory `eas.json` has an Android internal-distribution APK profile that bakes in `https://api.ragchatbot.dev` without changing Expo Go's ignored local-IP `.env`. The first signed Android EAS build `6402c25e-968c-40fe-a01b-9f9232b7d371` failed at native compile because Document Picker 56 was incompatible with Expo SDK 53 (ERR-084). The mobile dependency is now `expo-document-picker@13.1.6`; Expo Doctor passes 18/18, full typecheck/build and production Android export pass, and the bundle embeds the production API URL. Replacement signed Android build `9cfd93ee-e0d4-44d0-8c75-30734aa8b05a` was submitted from `5dcf7b2` and is queued. Once it finishes, download it, complete installed-device production acceptance, then create the public GitHub Release. See `docs/MOBILE_APK_RELEASE.md`.
...
- Failed-candidate restoration is demonstrated with a temporary health-response shim and existing immutable `7395…` images. The release script deployed the candidate, detected a revision mismatch, restored API/web `37e3…`, persisted that state, and reopened admission. Final public API/web and Valheim checks passed. The temporary verifier's stale string assertion is recorded as ERR-077; it did not affect the completed rollback.
...
- User completed the real web acceptance test without issues. Mobile login and live document ingestion/chat/source acceptance remain deferred. No browser surface is connected for automated UI verification.
- A drained operational rollback test rebuilt the identical VM archive as `rollback-test-20260916-1e81efb`, switched API/web to it, then restored `manual-60653da1fc21`. Candidate and restored health were DB-connected; local web returned 200 and Valheim remained up. `rag-20260916T055152-590682.dump` was taken before the switch. This proves the image-switch procedure, not a code-version rollback; repeat with two compatible GHCR releases after CI exists.
- `APP_REVISION` from the production env file had overridden the image revision in health responses. The Compose wrapper now accepts a tag-safe `APP_REVISION_OVERRIDE` through a private temporary second env file, so deployment tags and health evidence agree without changing VM secrets (ERR-070). The configuration must be transferred to Jarvis before the next image deployment.
- Added a SHA-pinned GitHub Actions workflow for Node 24 / pnpm 11.6.0 frozen install, typecheck, build and API/web production-image build. It pushes only immutable `ghcr.io/tengis01/rag-api:sha-<commit>` and `rag-web:sha-<commit>` images on `main` or manual dispatch; pull requests build but cannot publish. The first GitHub run, GHCR package creation and Jarvis pull/deploy remain pending.
- User reported the initial CI run completed without issue. Jarvis anonymous pulls of the `7395fe0...` image tags returned `unauthorized`, consistent with private GHCR packages. `scripts/deploy/pull-ghcr.sh` now pulls and verifies two exact full-SHA image labels after Docker has a RAG-specific `read:packages` credential; it never deploys by itself. Create that token/login next, then perform the drained manual GHCR release.
- User created the Jarvis-only GHCR credential. `37e3fca3f10de6b0ab1ebab79201c27b85f4fd38` API/Web images pulled and label-verified, then deployed with drain and fresh backup `rag-20260916T072841-627051.dump`. Final public API/Web health is 200 with revision `sha-37e3fca3f10de6b0ab1ebab79201c27b85f4fd38`; Postgres, Cloudflare and Valheim were untouched. This is the first real GHCR release; restricted SSH automation remains next.
- Prepared `scripts/deploy/release-ghcr.sh`, a lock/disk/pull/label/drain/backup/health/rollback release procedure, plus a forced-command SSH entrypoint and `workflow_dispatch`-only `.github/workflows/deploy.yml`. No deployment key or GitHub environment secret has been created yet; runner-to-VM SSH reachability and the first manual dispatch remain pending.
